...
public class TvsePage extends BasePage {
	...
	WebElement TVSEIcon1;

	@FindBy(id = "engineerBPID")
	...
	@FindBy(xpath = "//span[contains(@class,'details-label') and contains(text(),'Due Amount:')]")
	WebElement fetchedAmount;

	@FindBy(xpath = "//button[contains(@class,'button-primary') and contains(text(),'Submit')]")
	...
	@FindBy(xpath = "(//a[contains(text(), 'View All')]//img[@src='assets/Home/view_all.svg'])[3]")
	WebElement viewAll;
	...
	// Load all objects
	public TvsePage(WebDriver wdriver) {
		super(wdriver);
		PageFactory.initElements(wdriver, this);
	}

	// Perform action on page based on given commands
	public void  tvse(Map<String, String> usrData)
	...
	waitUntilElementIsClickableAndClickTheElement(TVSEIcon);
		//	commonUtils.selectCmsBiller();
			...
			System.out.println("TVSE icon clicked");
			waitUntilElementIsClickableAndClickTheElement(TVSEIcon1);
			System.out.println("second TVSE icon clicked");
		...
		if (usrData.get("GETAMOUNT").equalsIgnoreCase("YES")) {
				...
				System.out.println("Amount Details Fetched");
		}

				//commonUtils.waitForSpinner();

			/*	if (usrData.get("MOBILENUMBER").equalsIgnoreCase("123456")) {
					waitUntilElementIsVisible(toasterMsg);
					Assert.assertEquals(toasterMsg.getText(), "Invalid customer");
					...
					cmsDetailsFromIni("TvsefetchedAmount", replaceSymbols(amount));
				}
			//} else 
				if (usrData.get("GETAMOUNT").equalsIgnoreCase("Clear")) {
			...
			if (usrData.get("SUBMIT").equalsIgnoreCase("Yes")) {
				if (!usrData.get("AMOUNT").equalsIgnoreCase("SKIP")) {
					...
					cmsDetailsFromIni("StoreTvseAmount", usrData.get("AMOUNT"));
				}

				// Click on Submit button
				waitUntilElementIsClickableAndClickTheElement(SubmitButton);
				System.out.println("Tvse submit button clicked");
	...
	public void assertionOnSuccessScreen(Map<String, String> usrData)
		...
		Assert.assertEquals(cmsTxnScreenMessage.getText(), "Deposit to TVS Electronics successful");
		System.out.println(cmsTxnScreenMessage.getText());
