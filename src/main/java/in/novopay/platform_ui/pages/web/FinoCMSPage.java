...
public class FinoCMSPage extends BasePage {
	...
	WebElement finoCMSIcon;

	@FindBy(xpath = "//fino-collection//span[@class='selection']//span[@role='combobox']")
	WebElement clientDropdown;

	@FindBy(xpath = "//*[@type='search']")
	WebElement dropDownSearch;

	@FindBy(xpath = "//label[@for='UniqueID']")
	WebElement label;

	@FindBy(xpath = "//*[@id='UniqueID']")
	WebElement idField;
	...
	@FindBy(xpath = "//label[contains(text(),'MOBILE NUMBER')]/following-sibling::input")
	WebElement fetchedMobNum;

	@FindBy(xpath = "//fino-collection/div//button[contains(text(),'Proceed')]")
	WebElement finoProceedButton;
	...
	// Load all objects
	public FinoCMSPage(WebDriver wdriver) {
		super(wdriver);
		PageFactory.initElements(wdriver, this);
	}

	// Perform action on page based on given commands
	public void finoCMS(Map<String, String> usrData)
...
//			System.out.println(usrData.get("CLIENT") + " drop down selected");
			...
			idField.sendKeys(usrData.get("CLIENTID"));
			...
			if (usrData.get("PROCEED").equalsIgnoreCase("YES")) {
				...
				if (usrData.get("CLIENTID").equalsIgnoreCase("1000")) {
					...
					waitUntilElementIsVisible(fetchedMobNum);
			...
			if (usrData.get("SUBMIT").equalsIgnoreCase("Yes")) {
				if (!usrData.get("AMOUNT").equalsIgnoreCase("SKIP")) {
					waitUntilElementIsClickableAndClickTheElement(fetchedMobNum);
					fetchedMobNum.clear();
					Thread.sleep(1000);
					fetchedMobNum.sendKeys(usrData.get("MOBNUM"));
					cmsDetailsFromIni("StoreFinoCMSMobNum", usrData.get("MOBNUM"));
					fetchedMobNum.sendKeys(Keys.TAB);
					Thread.sleep(1000);
					fetchedAmount.click();
					fetchedAmount.clear();
					Thread.sleep(1000);
					fetchedAmount.sendKeys(usrData.get("AMOUNT"));
					cmsDetailsFromIni("StoreFinoCMSAmount", usrData.get("AMOUNT"));
				}

				// Click on Submit button
				waitUntilElementIsClickableAndClickTheElement(finoProceedButton);
